    ...
    private void ParseButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (_parsedRequest == null)
            {
                ...
                sb.AppendLine($"  'Do you want to': {(_extractedText.Contains("Do you want to", StringComparison.OrdinalIgnoreCase) ? "✓ FOUND" : "✗ MISSING")}");
                sb.AppendLine($"  Numbered options: {(System.Text.RegularExpressions.Regex.IsMatch(_extractedText, @"^\s*\d+[\.\)]", System.Text.RegularExpressions.RegexOptions.Multiline) ? "✓ FOUND" : "✗ MISSING")}");
                sb.AppendLine($"  'Yes' option: {(_extractedText.Contains("Yes", StringComparison.OrdinalIgnoreCase) ? "✓ FOUND" : "✗ MISSING")}");
                sb.AppendLine($"  'No' option: {(_extractedText.Contains("No", StringComparison.OrdinalIgnoreCase) ? "✓ FOUND" : "✗ MISSING")}");

                StatusTextBlock.Text = "FAIL: Claude prompt not detected.";
            }
            else
            {
                if (_parsedRequest.HasPersistentApprovalOption && _parsedRequest.PersistentApprovalOptionNumber.HasValue)
                {
                    sb.AppendLine("═══════════════════════════════════════");
                ...
                sb.AppendLine();
                sb.AppendLine($"Has Persistent Approval Option: {_parsedRequest.HasPersistentApprovalOption}");

                if (_parsedRequest.PersistentApprovalOptionNumber.HasValue)
                {
                    sb.AppendLine($"Persistent Approval Option Number: {_parsedRequest.PersistentApprovalOptionNumber.Value}");
                }
                else
                {
                    sb.AppendLine($"Persistent Approval Option Number: NOT DETECTED");
                }
                ...
                {
                    var marker = option.Number == _parsedRequest.PersistentApprovalOptionNumber ? "👉" : "  ";
                ...
                if (_parsedRequest.HasPersistentApprovalOption && _parsedRequest.PersistentApprovalOptionNumber.HasValue)
                {
                    sb.AppendLine();
                    sb.AppendLine("═══════════════════════════════════════");
                    sb.AppendLine($"✅ Ready for automation: Would send '{_parsedRequest.PersistentApprovalOptionNumber.Value}' + Enter");
                    sb.AppendLine("═══════════════════════════════════════");

                    StatusTextBlock.Text = $"PASS: Detected option {_parsedRequest.PersistentApprovalOptionNumber.Value} for automation.";
                }
                else
                {
                    sb.AppendLine();
                    sb.AppendLine("⚠️ Persistent approval option not found - automation would not execute.");

                    StatusTextBlock.Text = "Prompt detected but persistent approval option missing.";
                }
            }
        ...
        try
        {
            ...
            if (saveDialog.ShowDialog() == true)
            {
                ...
                if (_parsedRequest != null)
                {
                    ...
                    sb.AppendLine($"Prompt Type: {_parsedRequest.PromptType}");
                    sb.AppendLine($"Has Persistent Approval Option: {_parsedRequest.HasPersistentApprovalOption}");
                    sb.AppendLine($"Persistent Approval Option Number: {_parsedRequest.PersistentApprovalOptionNumber?.ToString() ?? "NULL"}");
                    sb.AppendLine();
